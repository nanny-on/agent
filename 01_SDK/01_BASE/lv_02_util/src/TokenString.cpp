...
CTokenString* t_TokenString = NULL;
...
CTokenString::CTokenString(BYTE* pbInput, INT32 nLen, char pTok, INT32 nReversPlay)
{
    m_pcBuffer = NULL;
    m_nStart = 0;
	m_nEnd = 0;
...
CTokenString::CTokenString(const char *pcInput, INT32 nLen, char pTok, INT32 nReversPlay)
{
    m_pcBuffer = NULL;
    m_nStart = 0;
	m_nEnd = 0;
...
void    CTokenString::SetBuffer(const char *pcInput, INT32 nLen, char cTok, INT32 nReversPlay)
{
...
INT32	CTokenString::IsNext_Line()
{
    if(m_nPos >= m_nEnd || m_nPos < m_nStart)
        return 0;
    return 1;
...
String		CTokenString::NextToken_Query()
{    
	String  strRtn;
	BOOL bContinue = TRUE;
...
String		CTokenString::NextToken_Line()
{    
    String  strRtn;
    BOOL    bContinue = TRUE;
    BOOL	bTokenFine = FALSE;
...
String  CTokenString::RemainValue()
{
    String  strRtn;
    BOOL bContinue = TRUE;
...
UINT32  CTokenString::NextToken_UInt()
{
    String strNextValue = NextToken();
    if(strNextValue.length())
    {
        return (UINT32)StrToInt64(strNextValue.c_str());
    }
    return 0;
}
//-------------------------------------------------------------------------

INT32   CTokenString::NextToken_Int()
{
    String strNextValue = NextToken();
    if(strNextValue.length())
    {
        return (UINT)StrToInt64(strNextValue.c_str());
    }
    return 0;
}
//-------------------------------------------------------------------------

UINT64		CTokenString::NextToken_UInt64()
{
    String strNextValue = NextToken();
    if(strNextValue.length())
    {
        return (UINT64)StrToInt64(strNextValue.c_str());
    }
    return 0;
}
//-------------------------------------------------------------------------

BOOL    CTokenString::NextToken_Bool()
{
    return (NextToken_Int() == 0 ? false : true);
}
//-------------------------------------------------------------------------

INT16   CTokenString::NextToken_Short()
{
    return (INT16)NextToken_Int();
}
//-------------------------------------------------------------------------

UINT8    CTokenString::NextToken_Byte()
{
    return (UINT8)NextToken_Short();
}
//-------------------------------------------------------------------------

INT32	CTokenString::EditToken(char pTok)
{
	m_cTok = pTok;
	return 0;
}
...
UINT32	CTokenString::GetRemainTokenCount()
{
	UINT32 nRtn = 0;
	INT32 nPos = m_nPos;
...
String	CTokenString::NextToken_Len(INT32 nLen)
{
    String  strRtn;
	BOOL bContinue = TRUE;
...
String  CTokenString::NextTokenByTokPos(INT32 nTokPos)
{
	String  strRtn;
	BOOL bContinue = TRUE;
	INT32 nPos = nTokPos;

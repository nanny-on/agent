...
INT32	CThreadAccNotifyEvent::InitSockEnv(INT32 &nClientFd)
{
	INT32 nRetVal = 0;
	INT32 nClientLen = 0;
	...
	do{
		m_nAccClientFd = socket(AF_UNIX, SOCK_STREAM, 0);
		if (m_nAccClientFd == -1)
		{
			nRetVal = -2;
			break;
		...
		nRetVal = connect(m_nAccClientFd, (struct sockaddr *)&stClientAddr, nClientLen);
		if (nRetVal < 0)
		{
			nRetVal = -3;
			break;
		}
		nClientFd = m_nAccClientFd;
		m_bIsConnected = TRUE;
		m_nSendTime = 0;
		nRetVal = 0;
	}while(FALSE);
...
VOID	CThreadAccNotifyEvent::GetClientFd(INT32 &nClientFd)
{
	pthread_mutex_lock (&m_SockMutex);
	nClientFd = m_nAccClientFd;
	pthread_mutex_unlock (&m_SockMutex);
...
BOOL	CThreadAccNotifyEvent::IsSockConnected()
{
...
VOID	CThreadAccNotifyEvent::UninitSockEnv()
{
	pthread_mutex_lock (&m_SockMutex);
	if(m_nAccClientFd != -1)
	{
		close(m_nAccClientFd);
		m_nAccClientFd = -1;
	}
	m_nSendTime = 0;
	m_bIsConnected = FALSE;
...
INT32 CThreadAccNotifyEvent::SockRecv(INT32 nFd, PVOID pRecvData, INT32 nReqSize)
{
...
INT32 CThreadAccNotifyEvent::SockWrite(INT32 nFd, PVOID pWriteData, INT32 nReqSize)
{
...
UINT32 CThreadAccNotifyEvent::GetSendTime()
{
	UINT32 nTime = 0;
	pthread_mutex_lock (&m_SockMutex);
	nTime = m_nSendTime;
...
INT32 CThreadAccNotifyEvent::SendAliveMessage()
{
...
INT32	CThreadAccNotifyEvent::InitAccNotifyEvent(INT32 &nNotifyFd, INT32 &nClientFd, INT32 nTestMode)
{
	INT32 i, nRetVal = 0;
	INT32 nNFd = -1;
	INT32 nCFd = -1;
	...
	do{
		...
		if(nRetVal < 0 || nCFd == -1)
		{
			if(t_AccNotifyDlg != NULL)
				t_AccNotifyDlg->WriteLogE("fail to init sock env (%d) (%d)", nRetVal, errno);
			nRetVal -= 20;
			break;
		}
		...
		nRetVal = AddWatchAccNotify(1, ASI_ACCNOTIFY_PATH);
		if(nRetVal < 0)
		{
			nRetVal -= 100;
			break;
...
INT32 CThreadAccNotifyEvent::Run() 
{
	...
	m_nPause = 1;
	nRetVal = InitAccNotifyEvent(nNotifyFd, nClientFd, nTestMode);
	if(nRetVal == 0)
	{
		m_nPause = 0;
		nRetVal = SendReqPoliyToServer(nClientFd, pChkFileProc);
		if(nRetVal < 0)
			m_nPause = 1;
	}
	...
	while(GetContinue())
	{
		if(!m_nPause)
		{
			...
			if(nTestMode == 0)
				nRetVal = CheckAccNotifyFile(nNotifyFd, nClientFd, pcBuffer, pChkFileProc);
			else
				nRetVal = AnalyzeAccNotifyEvent2(nClientFd, pChkFileProc, nTestCount++%6);
			if(nRetVal != 0)
			{
				if(nRetVal == -1)
				{
...
INT32 CThreadAccNotifyEvent::AddAccNotifyMark(PFANOTIFY_PATH pAccNotifyPath)
{
	INT32 nNotifyFd = -1;
	INT32 nRetVal = 0;
	...
	{
		if(t_AccNotifyDlg)
			t_AccNotifyDlg->WriteLogE("[AddWatchAccNotify] invalid accnotify fd");
		return -2;
	}
	...
	{
		if(t_AccNotifyDlg)
			t_AccNotifyDlg->WriteLogE("[AddWatchAccNotify] fail to add accnotify watch [%s] : [%d]", pAccNotifyPath->acNotifyPath, errno);
		return -3;
	}
...
INT32 CThreadAccNotifyEvent::AddWatchAccNotify(INT32 nOrderID, char *pPath)
{
	INT32 nLen = 0;
	INT32 nRetVal = 0;
	...
	nRetVal = AddAccNotifyMark(pAccNotifyPath);
	if(nRetVal != 0)
	{
		nRetVal -= 10;
...
BOOL	CThreadAccNotifyEvent::GetFilePathFromFd(INT32 nFd, PASI_CHK_INFO pFileInfo)
{
	...
	if(_stricmp(pFileInfo->acPath, ASI_ACCNOTIFY_PATH))
	{
		return FALSE;
	}
	return TRUE;
...
INT32	CThreadAccNotifyEvent::SendEventToServer(INT32 nNotifyFd, INT32 nClientFd, PVOID pMetaData, PASI_CHK_FILE_PROC pChkFileProc)
{
...
INT32	CThreadAccNotifyEvent::SendReqPoliyToServer(INT32 nClientFd, PASI_CHK_FILE_PROC pChkFileProc)
{
	INT32 i, nRetVal = 0;
	INT32 nSize = sizeof(ASI_CHK_FILE_PROC);
	CAccNotifyDlg* t_AccNotifyDlg = (CAccNotifyDlg *)m_pNotifyWnd;
...
INT32	CThreadAccNotifyEvent::AnalyzeAccNotifyEvent(INT32 nNotifyFd, INT32 nClientFd, PVOID pMetaData, PASI_CHK_FILE_PROC pChkFileProc)
{
...
INT32		CThreadAccNotifyEvent::CheckAccNotifyFile(INT32 nNotifyFd, INT32 nClientFd, char *pcBuffer, PASI_CHK_FILE_PROC pChkFileProc)
{
	INT32 nRetVal = 0;
	ssize_t nLength = 0;
	...
	if (stPollFd.revents & POLLIN)
	{
		...
		if (nLength > 0)
		{
			...
			{
				nRetVal = AnalyzeAccNotifyEvent(nNotifyFd, nClientFd, (PVOID)pMetaData, pChkFileProc);
				if(nRetVal < 0)
				{
					nRetVal -= 100;
					return nRetVal;
...
INT32	CThreadAccNotifyEvent::AnalyzeAccNotifyEvent2(INT32 nClientFd, PASI_CHK_FILE_PROC pChkFileProc, INT32 nCount)
{
	INT32 nRetVal = 0;
	INT32 nAcVal = RET_NONE;
	INT32 nSize = sizeof(ASI_CHK_FILE_PROC);
	...
	do{
		...
		if(nCount == 0)
			...
			strncpy(pChkFileProc->stFileInfo.acFullPath, "/bin/zsh", MAX_PATH-1);
		else if(nCount == 4)
			strncpy(pChkFileProc->stFileInfo.acFullPath, "/bin/touch", MAX_PATH-1);
		else if(nCount == 5)
			strncpy(pChkFileProc->stFileInfo.acFullPath, "/bin/tar", MAX_PATH-1);
		else
		{
			nAcVal = RET_NONE;
			nRetVal = 0;

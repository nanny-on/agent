...
INT32	CMonTimerUtil::EditTimer(UINT32 nTimerID, UINT32 nInterval, UINT32 nMsgType, UINT32 nEnabled, INT32 nLimited)
{
	...
	do{
		...
		{
			if(t_NotifyDlg != NULL)
				t_NotifyDlg->WriteLogE("[EditTimer] fail to find timer (%d)", nTimerID);
			nReVal = -1;
			break;
		}
...
INT32	CMonTimerUtil::EnableTimer(UINT32 nTimerID, UINT32 nResetTime)
{
	...
	do{
		...
		{
			if(t_NotifyDlg != NULL)
				t_NotifyDlg->WriteLogE("[EnableTimer] fail to find timer (%d)", nTimerID);
			nReVal = -1;
			break;
		}
...
INT32	CMonTimerUtil::DisableTimer(UINT32 nTimerID, UINT32 nResetTime)
{
	...
	do{
		...
		{
			if(t_NotifyDlg != NULL)
				t_NotifyDlg->WriteLogE("[DisableTimer] fail to find timer (%d)", nTimerID);
			nReVal = -1;
			break;
		}
...
void	CMonTimerUtil::ChkTimer()
{
	...
	for(begin; begin != end; begin++)
	{
		...
		if(t_NotifyDlg != NULL)
			t_NotifyDlg->PostMessage(m_nNotifyID, nChkTimerID, 0);

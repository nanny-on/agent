...
{
	if(nSig == SIGTERM && t_FaNotifydDlg)
	{
		t_FaNotifydDlg->CloseMain();
		Sleep(500);
		t_FaNotifydDlg->PostMessage(WM_FANOTIFYD_CLOSE, 0, 0);
	}
}
...
int make_daemon(void)
{
	...
	ret = check_proc_exist_by_name(FANOTIFYD_IDENT, getpid());
	if (ret == 1)
	{
		fprintf(stdout, "fanotifyd is already running.\n\n");
		return -2;
	}
...
int main(int argc, char* argv[])
{
	...
	t_FaNotifydDlg = new CFaNotifyDlg();
	if(t_FaNotifydDlg == NULL)
	{
		fprintf(stdout, "fail to allocate class (%d).\n", errno);
		exit(3);
	}
	t_FaNotifydDlg->Run();
	SAFE_DELETE(t_FaNotifydDlg);
	exit(0);
}

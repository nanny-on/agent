...
INT32 write_pgm(char *acFullPath)
{
	...
	if(acFullPath == NULL || acFullPath[0] == 0)
		return -1;
	...
	{
		nRetVal = execlp("/bin/cp_test", "cp_test", "/bin/ls", acFullPath, NULL);
		if(nRetVal < 0)
		{
			get_current_date_time(acTime);
			fprintf(stderr, "[%s] deny to write %s\n", acTime, acFullPath);
		}
		exit(127);
	}
	else
...
void print_usage()
{
	...
	fprintf(stdout, "\twrite_test_pgm -d path -c file_count [-s sleep(ms)]\n");
}
